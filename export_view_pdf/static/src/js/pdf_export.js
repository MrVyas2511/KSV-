...
//  method to define the functionality when clicking on the export pdf menu
    async onDirectExportPdf() {
        ...
        const exportField = [];
        let exportedFields = Object.values(this.env.model.config.fields).map(field => ({
        ...
        debugger;
        exportedFields = exportedFields.filter(item =>  !['id','create_uid','create_date','write_uid','write_date'].includes(item.name));
    ...
    async onDirectExportKhatavahiPdf() {

        if (this.env.model.config.resModel !== "voucher.register") {
            alert("This feature is only available for Voucher Register");
            return;
        }
        this.env.searchModel.trigger('direct-export-pdf');
        const exportField = [];
        ...
        this.env.model.dialog.add(ExportDialog, {
                ...
                confirm: async () => {
                    ...
                    var length_field = Array.from(Array(exportField.length).keys());
                    await rpc('/get_khatavahi_data',{
                        'model':this.env.model.config.resModel,
                        'res_ids':resIds.length > 0 && resIds,
                        'fields':exportField,
                        'grouped_by':['account_name'],
                        'context': this.env.model.config.context,
                        'domain':this.env.model.config.domain,
                    }).then(data => {
                        if (this.env.model.config.groupBy[0]){
                            var action = {
                                'type': 'ir.actions.report',
                                'report_type': 'qweb-pdf',
                                'report_name':'export_view_pdf.export_in_pdf_khatavahi',
                                'data':{'length':length_field,'group_len':[0,1,2,3],'record':data,}
                            };
                        }
                        else{
    ...
    async onDirectExportConsumablePdf() {

        if (this.env.model.config.resModel !== "consumable.register") {
            alert("This feature is only available for Consumable Register");
            return;
        }
        ...
        this.env.model.dialog.add(ExportDialog, {
                ...
                confirm: async () => {
                    ...
                    await rpc('/get_data',{
                        ...
                        'res_ids':resIds.length > 0 && resIds,
                        'fields':exportField,
                        'grouped_by':['date:day', 'account_name'],
                        'context': this.env.model.config.context,
                        'domain':this.env.model.config.domain,
                    }).then( data => {
                        var action = {
                            'type': 'ir.actions.report',
                            'report_type': 'qweb-pdf',
                            'report_name':'export_view_pdf.export_in_pdf_rojmel',
                            'data':{'length':length_field,'group_len':[0,1,2,3],'record':data,}
                        };
                        return this.env.model.action.doAction(action);
                    });
